...
# Keep service commands on this account's user manager with the runtime HOME/PATH.
alias openclaw-status='env HOME=$OPENCLAW_HOME PATH=$RUNTIME_PATH XDG_RUNTIME_DIR=$OPENCLAW_RUNTIME_DIR DBUS_SESSION_BUS_ADDRESS=unix:path=$OPENCLAW_RUNTIME_DIR/bus systemctl --user status openclaw-$AGENT_ID.service'
alias openclaw-start='env HOME=$OPENCLAW_HOME PATH=$RUNTIME_PATH XDG_RUNTIME_DIR=$OPENCLAW_RUNTIME_DIR DBUS_SESSION_BUS_ADDRESS=unix:path=$OPENCLAW_RUNTIME_DIR/bus systemctl --user start openclaw-$AGENT_ID.service'
alias openclaw-stop='env HOME=$OPENCLAW_HOME PATH=$RUNTIME_PATH XDG_RUNTIME_DIR=$OPENCLAW_RUNTIME_DIR DBUS_SESSION_BUS_ADDRESS=unix:path=$OPENCLAW_RUNTIME_DIR/bus systemctl --user stop openclaw-$AGENT_ID.service'
alias openclaw-restart='env HOME=$OPENCLAW_HOME PATH=$RUNTIME_PATH XDG_RUNTIME_DIR=$OPENCLAW_RUNTIME_DIR DBUS_SESSION_BUS_ADDRESS=unix:path=$OPENCLAW_RUNTIME_DIR/bus systemctl --user restart openclaw-$AGENT_ID.service'
alias openclaw-logs='journalctl --user -u openclaw-$AGENT_ID.service -f'
alias openclaw-nats-status='env HOME=$OPENCLAW_HOME PATH=$RUNTIME_PATH XDG_RUNTIME_DIR=$OPENCLAW_RUNTIME_DIR DBUS_SESSION_BUS_ADDRESS=unix:path=$OPENCLAW_RUNTIME_DIR/bus systemctl --user status $${NATS_SVC_NAME}.service'
alias openclaw-nats-restart='env HOME=$OPENCLAW_HOME PATH=$RUNTIME_PATH XDG_RUNTIME_DIR=$OPENCLAW_RUNTIME_DIR DBUS_SESSION_BUS_ADDRESS=unix:path=$OPENCLAW_RUNTIME_DIR/bus systemctl --user restart $${NATS_SVC_NAME}.service'
