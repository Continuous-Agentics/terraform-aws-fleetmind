...
| <a name="input_node_version"></a> [node\_version](#input\_node\_version) | Node.js major version to install on FleetMind agent instances via NodeSource RPMs (major only, e.g. "24"). Passed straight through to NodeSource's setup_<major>.x script (curl -fsSL https://rpm.nodesource.com/setup_<major>.x \| bash -, then dnf install -y nodejs) in modules/agent/user\_data/agent\_bootstrap.sh.tpl. Only "24" is supported today. | `string` | `"24"` | no |
