...
Workspace path: `/home/openclaw/.openclaw/workspace/<agent>` — also HOME for the Slack plugin, gateway, and NATS subscriber (`.openclaw` lives there, under the `openclaw` account's home). Matches the standard `~/.openclaw` layout FleetMind's local/ssh targets already use.
