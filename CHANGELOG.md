...
- Bootstrap agents with the practical `openclaw` runtime baseline: an idempotent `/home/openclaw` Bash user with Node/npm, Docker-group access, and lingering; gateway and NATS now run as that user's shared-environment systemd user services rather than as root-managed `ec2-user` services. FleetMind CLI and template companion changes are required for pull-self restart compatibility.
